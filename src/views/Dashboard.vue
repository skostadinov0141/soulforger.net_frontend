<template>
	<div style="padding: 80px">
		<p>
			...
			<br />
			Lebenspunkte -> {{ character.baseStats.lifePoints.current }} /
			{{ character.baseStats.lifePoints.default }} <br />
			Kraft -> {{ character.baseStats.strength.current }} /
			{{ character.baseStats.strength.default }} <br />
			Akrobatik -> {{ character.baseStats.acrobatics.current }} /
			{{ character.baseStats.acrobatics.default }} <br />
			Wahrnehmung -> {{ character.baseStats.preception.current }} /
			{{ character.baseStats.preception.default }} <br />
			Charisma -> {{ character.baseStats.charisma.current }} /
			{{ character.baseStats.charisma.default }} <br />
			Wille -> {{ character.baseStats.will.current }} /
			{{ character.baseStats.will.default }} <br />
			Verbundenheit -> {{ character.baseStats.connection.current }} /
			{{ character.baseStats.connection.default }} <br />
      ...
      Wille x Verbundenheit -> {{ character.baseStats.wcx }} <br> -->
		...
		<div
			style="margin-top: 40px; display: flex; flex-direction: column; gap: 8px"
		>
...
const heroSections = ref([
	{
		...
		content:
			"Soulforger ist eine digitale Plattform, die darauf abzielt, die Komplexität von Rollenspielen zu verringern. Sie unterstützt mehrere Regelwerke mit dem gleichen Funktionsumfang für jedes einzelne.",
	},
	{
		title: "Was ist Soulforger?",
		content:
...
const rulebooks = ref([
	...
	},
	{
		title: "Nirve",
		description:
			"In Nirvé schreibst du die Sagen hinter den Legenden. Seit dem Ende der Verwüstung liegt die Welt allein in den Händen der Humaner. Religionen kämpfen um die Wahrheit, Reiche um Vorherrschaft. Jahrhunderte der Geschichte sind bereit, von dir und deinen Weggefährten erlebt zu werden...",
